...
try {
 ...
 const symbol=symbols.symbol?.find(s=>s.symbolName==='XAUUSD'||s.symbolName==='XAU/USD');if(!symbol)throw new Error('BROKER_XAUUSD_NOT_FOUND');
 ...
 const end=Math.floor(Date.now()/3600000)*3600000, start=end-365*86400000;
 ...
 const result={source:'CTRADER_BROKER',instrument:'XAU/USD',brokerSymbol:symbol.symbolName,symbolId:symbol.symbolId,timeframe:'H1',requestedFrom:start,requestedTo:end,fetchedAt:Date.now(),synthetic:false,spec:{digits:spec.digits,pipPosition:spec.pipPosition,lotSize:spec.lotSize,minVolume:spec.minVolume,maxVolume:spec.maxVolume,stepVolume:spec.stepVolume},candles};
