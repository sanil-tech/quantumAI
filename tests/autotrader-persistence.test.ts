    ...
    const handleQuery = async (text: string, params?: any[]) => {
      ...
      if (text.includes('INSERT INTO positions')) {
        ...
        const row = {
          ...
          timeframe: params![5] ?? existing.timeframe ?? 'M15',
          direction: params![6] ?? existing.direction,
          quantity: String(params![7] ?? existing.quantity ?? '0.10'),
          entry_price: String(params![8] ?? existing.entry_price ?? '1.0'),
          current_price: String(params![9] ?? existing.current_price ?? '1.0'),
          close_price: params![10] != null ? String(params![10]) : existing.close_price || null,
          stop_loss: params![11] != null ? String(params![11]) : existing.stop_loss || null,
          take_profit: params![12] != null ? String(params![12]) : existing.take_profit || null,
          take_profit_2: params![13] != null ? String(params![13]) : existing.take_profit_2 || null,
          unrealized_profit: String(params![14] ?? existing.unrealized_profit ?? '0'),
          realized_profit: String(params![15] ?? existing.realized_profit ?? '0'),
          pnl_pips: String(params![16] ?? existing.pnl_pips ?? '0'),
          commission: String(params![17] ?? existing.commission ?? '0'),
          swap: String(params![18] ?? existing.swap ?? '0'),
          status: params![19] ?? existing.status ?? 'OPEN',
          close_reason: params![20] ?? (existing.close_reason || null),
          broker: params![21] ?? (existing.broker || 'CTRADER'),
          environment: params![22] ?? (existing.environment || 'DEMO'),
          proposal_id: params![23] ?? (existing.proposal_id || null),
          approval_id: params![24] ?? (existing.approval_id || null),
          strategy_id: params![25] ?? (existing.strategy_id || null),
          strategy_version: params![26] ?? (existing.strategy_version || null),
          learning_version: params![27] ?? (existing.learning_version || '1.0'),
          broker_order_id: params![28] ?? (existing.broker_order_id || null),
          broker_position_id: params![29] ?? (existing.broker_position_id || null),
          broker_deal_id: params![30] ?? (existing.broker_deal_id || null),
          reconciliation_status: params![31] ?? (existing.reconciliation_status || 'MATCHED'),
          idempotency_key: params![32] ?? (existing.idempotency_key || null),
          opened_at: params![33] ?? (existing.opened_at || new Date()),
          closed_at: params![34] ?? (existing.closed_at || null)
        };
