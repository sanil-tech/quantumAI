...
import { enhancedVetoLogic } from './enhancedVetoLogic';
...
export class AutonomousTradeExecutor {
  ...
  /**
   * Main autonomous loop - Steps 1-10 WITH ENHANCED REAL-TIME VETO LOGIC
   */
  private async runTradeLoop() {
    try {
      ...
      // ✅ NEW: ENHANCED REAL-TIME VETO CHECK
      const setupType = `${signal.direction}_${this.config.pair}`;
      ...
      await enhancedVetoLogic.recordVetoDecision(
        vetoAnalysis,
        `veto_${Date.now()}`
      );

      // Check veto recommendation
      if (vetoAnalysis.recommendation === 'VETO') {
        console.log(`❌ [ENHANCED VETO] Trade blocked: ${vetoAnalysis.decision.explanation}`);
        this.scheduleNext();
        return;
      }

      if (vetoAnalysis.recommendation === 'CAUTION') {
        console.log(`⚠️ [CAUTION] Trade allowed with warning: ${vetoAnalysis.decision.explanation}`);
      }
      ...
      if (decision.status !== 'APPROVED' || !decision.token) {
        console.log(`❌ [GOVERNANCE VETO] Trade rejected:`, decision.rejection_reasons);
        this.scheduleNext();
        return;
      }
  ...
  /**
   * STEP 5: Execute trade - with broker execution
   */
  ...
  /**
   * STEP 6-7: Monitor position until closed - with real-time SL/TP detection
   */
  ...
  /**
   * Close position with broker and database
   */
