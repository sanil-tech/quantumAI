...
export type ReplayConfig = {initialEquity:number;riskPct:number;minConfidence:number;pendingBars:number;holdingBars:number;spec:Spec;costs:Costs};
 ...
 for(let i=startIndex;i<endIndex;i++){
  ...
  if(open){const buy=open.c.direction==='BUY',lo=b.low+(buy?0:config.costs.spread),hi=b.high+(buy?0:config.costs.spread),op=b.open+(buy?0:config.costs.spread);
   ...
   if(stopHit){const stopPrice=filledNow?open.stop:(buy?Math.min(op,open.stop):Math.max(op,open.stop));close(stopPrice+(buy?-1:1)*config.costs.slippage,'STOP_OR_BE',i);}
    ...
    if(open.remaining===1&&(buy?hi>=open.c.tp1:lo<=open.c.tp1)){const pnl=exitLeg(open.c.tp1,0.5);mark(pnl);open.banked+=pnl;open.remaining=0.5;open.stop=open.c.entry;
     // Conservative intrabar ambiguity after TP1: BE takes priority over TP2.
     if(buy?lo<=open.stop:hi>=open.stop)close(open.stop+(buy?-1:1)*config.costs.slippage,'TP1_THEN_BE',i);
    }
